...
\abstract{
	...
	We show that replacing traditional cubic splines with Chebyshev polynomials for interpolation in $k$ and $\ell$ improves performance and precision,
	so fewer perturbation equations and line-of-sight integrals can be solved for equal or higher precision.
	Chebyshev interpolation converges (up to) exponentially fast for smooth functions,
	which pairs particularly well with approximation-free codes.
	Considering a set of example source functions and angular power spectra for matter and the cosmic microwave background (CMB),
	we find anywhere from $0$-$4$ orders of magnitude lower interpolation error for the same number of points.
	For a typical CMB temperature power spectrum computed with interpolation in both $k$ and $\ell$,
	Chebyshev polynomials converge to $10^{-4}$-$10^{-5}$ relative error with only $50$-$80$ points per dimension,
	while cubic splines approach $10^{-4}$ error with $200$ points -- translating to a $4\times$ speedup.
	The exact improvement depends on the target function and becomes more dramatic for higher precision.
	We also give a practically oriented introduction to Chebyshev interpolation along with some usage strategies.
	The feature is implemented in SymBoltz (version 1.6),
...
In open and closed universes,
the perturbations depend on the \enquote{generalized wavenumber} $q(k)$ instead of $k$.
Generalizing Chebyshev interpolation here should be straightforward by interpolating in $q$ instead of $k$.
Additionally, the spherical Bessel functions $j_\ell(x)$
-- which we needed at both integer and non-integer $\ell$ --
generalize to hyperspherical Bessel functions $\Phi^\nu_\ell(x)$,
that depend on an additional curvature-dependent argument $\nu$ in addition to the multipole $\ell$.
Special methods have been devised for computing them for integer $\ell$ and $\nu$ using polynomial or recurrence relations \cite{lesgourguesFastAccurateCMB2014a,tramComputationHypersphericalBessel2017a},
but Chebyshev interpolation would require $\Phi_\ell^\nu(x)$ also at non-integer $\ell$,
This requires additional work to generalize this computation to non-integer $\ell$,
but one could fall back to integrating the ODE that defines $\Phi^\nu_\ell(x)$.
