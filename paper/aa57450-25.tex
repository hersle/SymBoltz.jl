...
This paper is as of SymBoltz version 1.0.0.
...
SymBoltz generates them automatically for the solver without distracting the modeler from focusing on the physical equations.
...
The code is as of SymBoltz version 1.0.0 and available in a notebook in the project repository.
...
For example, \texttt{sol(1.0, 2.0, g.Φ+g.Ψ)} computes $\Phi+\Psi$ at $k = 1 \, H_0/c$ and $\tau = 2 \, H_0^{-1}$ by expanding $\Psi$ in terms of $\Phi$ and other unknowns.
...
This is all the user must do.
The modification consists of writing down the equations verbatim,
...
While not groundbreaking, these results show that the differentiable pipeline works.
