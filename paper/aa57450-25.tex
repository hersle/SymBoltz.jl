...
\newunicodechar{ₐ}{\ensuremath{{}_\texttt{a}}}
...
This suggests that the $\Lambda$ cold dark matter (\LCDM{}) model is incomplete, despite it successfully explaining many observations.
...
Linear Einstein-Boltzmann solvers (\enquote{Boltzmann codes}), such as CAMB \citep{lewisEfficientComputationCMB2000a} and CLASS \citep{lesgourguesCosmicLinearAnisotropy2011}, are among the most essential tools in the cosmological modeling toolbox.
...
Emulators have become a popular proxy that trains differentiable neural networks to reproduce the output of nondifferentiable codes \citep[e.g.,][]{bollietHighaccuracyEmulatorsObservables2024,boniciCapsejlEfficientAutodifferentiable2024}.
...
For example, it predicts cosmic microwave background (CMB), baryon acoustic oscillation (BAO), or supernova (SN) observations, and it generates initial conditions for nonlinear $N$-body simulations of large-scale structure.
...
Organizing a code with a suitable modular structure that scales well in model space.
...
Fourth, the size of differential equations is very large, particularly for accurate descriptions of relativistic species.
...
For example, to implement a new species, the user must often modify the code in many places: input handling for new parameters, new background equations, new thermodynamics equations, new perturbation equations, joining each of these stages, output handling, and so on.
...
Even if unused components are deactivated by "if" statements at runtime, their mere presence in the source code increases its complexity, reduces its readability, and makes it harder to add more models.
...
However, they put much more strain on modelers to derive and validate several versions of the equations in different regimes.
...
They also complicate the numerics: time series from each ODE solution must be stitched together, each separate ODE system can use different tolerances, ODE integrators must be reinitialized, and so on.
...
\begin{figure*}
    ...
    \end{tikzpicture}
    \caption{SymBoltz represents cosmological models with symbolic equations grouped in physical components for the metric, gravity, and particle species. This is compiled to a numerical problem that splits equations into background and perturbation stages and generates fast code for ODE functions $\boldsymbol{f}$ and Jacobians $\boldsymbol{J}$. The problem is then solved, and the result is stored in a solution object that gives access to any variable $S$ in the model.}
...
Broadly speaking, the model defines what symbolic equations should be solved; the problem decides how they are solved; and the solution holds what was solved.
...
With knowledge of the symbolic equations, SymBoltz analyzes their structure programmatically to ease model specification, automate mechanical boilerplate tasks, and generate fast and stable code that avoids approximations.
...
SymBoltz defines a full \LCDM{} model in only 277 straightforward lines of code,%
...
SymBoltz generates them automatically and provides them to the solver without distracting the modeler from focusing on the equations.
...
This is friendlier to the modeler, who now simply has to provide one single set of equations, instead of deriving, implementing, and validating approximations.
...
Being free from convergence issues on this nonlinear problem is a major upside and suits the linear perturbation ODEs.
...
For example, calling \texttt{sol(1.0, 2.0, g.Φ+g.Ψ)} computes $\Phi+\Psi$ at $k = 1 \, H_0/c$ and $\tau = 2 \, H_0^{-1}$ by expanding it in terms of unknowns using the equations in Appendix \ref{sec:gravity}.
...
\begin{figure*}[ht!]
    ...
    \caption{Matter and CMB (TT, TE, and EE) power spectra computed by SymBoltz (SB; colored lines) compared to CLASS (CL; gray dashes) with relative errors $P_k^\text{SB}/P_k^\text{CL}-1$ and $C_l^\text{SB}/C_l^\text{CL}-1$ for the $w_0 w_a \text{CDM}$ model. CLASS uses the precision parameters in Appendix \ref{sec:precision}.}
    \label{fig:spectra}
\end{figure*}
...
Unless otherwise stated, temperatures are in $\mathrm{K}$ and other variables are in units in which \enquote{$G = c = H_0 = 1$.}
...
Initial conditions are fully ionized fractions $X_\Hy^+ = X_\He^+ = 1$, temperatures $T_b = T_\gamma$ ($\Delta T = 0$) in thermal equilibrium, the arbitrary $\kappa = 0$, and adiabatic perturbations $\delta/(1+w) = -3 \Psi / 2$ and $\theta = k^2 \tau \Psi / 2$.
