...
The modeler simply provides their equations, and SymBoltz solves them while eliminating friction in the modeling process.
...
Performance remains as good as in existing approximation-based codes due to efficient high-order implicit methods, fast generated code, optimal handling of the Jacobian, and sparse matrix methods.
...
Cosmology is at a crossroads \citep{freedmanCosmologyCrossroads2017}, as the rising precision of modern observations continues to reveal tensions with theory.
This suggests that the $\Lambda$ cold dark matter (\LCDM{}) model is not the ultimate truth, despite its successful explanation of many observations.
On the theoretical side, this situation drives a search for the \enquote{true} cosmological model through modifications to \LCDM{} \citep{bull$L$CDMProblemsSolutions2016}.
This exploration benefits from access to numerical tools that are easy to modify, which encourages relaxing model-dependent approximations, creating user-friendly interfaces, and structuring codes with modular components.
Some of the most important such tools in the cosmological modeling toolbox are linear Einstein-Boltzmann solvers (\enquote{Boltzmann codes} for short), such as CAMB \citep{lewisEfficientComputationCMB2000a} and CLASS \citep{lesgourguesCosmicLinearAnisotropy2011}.
...
They use both the likelihood and its gradient to explore parameter space more efficiently.
Emulators have become a popular \enquote{proxy} to do this from existing nondifferentiable codes by training differentiable neural networks to reproduce their output \citep[e.g.,][]{bollietHighaccuracyEmulatorsObservables2024,boniciCapsejlEfficientAutodifferentiable2024}.
...
A differentiable Boltzmann solver is therefore a cornerstone in the modern cosmological modeling toolbox.
...
For example, it predicts cosmic microwave background (CMB), baryon acoustic oscillation (BAO), or supernova (SN) observations, or it generates initial conditions for nonlinear $N$-body simulations of large-scale structure.

This article is structured as follows.
Section \ref{sec:intro} motivates SymBoltz by reviewing the history of Boltzmann codes and gradient methods.
Section \ref{sec:features} describes the structure and main features of SymBoltz relative to other codes.
Section \ref{sec:examples} shows example use.
Section \ref{sec:performance} compares performance to another code.
Section \ref{sec:futurework} describes paths for future work.
Section \ref{sec:conclusion} concludes with the current state of the code.
Appendices \ref{sec:implementation}, \ref{sec:precision} and \ref{sec:testing} list equations and implementation, comparison, and testing details.


\label{sec:intro}

Historically, \cite{peeblesPrimevalAdiabaticPerturbation1970} were the first to numerically integrate a comprehensive set of linear Einstein-Boltzmann equations.
Their work was refined over several years, and \cite{maCosmologicalPerturbationTheory1995} laid the groundwork for modern Boltzmann solvers with the COSMICS code.
\cite{seljakLineSightApproach1996} soon realized that photon multipoles can be integrated by parts to reduce thousands of coupled differential equations to independent integral solutions.
As first implemented in their code CMBFAST, this line-of-sight integration method has become a standard technique that greatly speeds up the calculation, but requires truncating the multipoles in the differential equations.
...
Any output from Einstein-Boltzmann codes are derived from the solution of these ODEs, such as the matter and CMB power spectra.

However, several properties of the equations complicate this task.
First, the equations are separated into computational stages that benefit substantially from being solved sequentially for performance and stability, such as the background, perturbations, and line-of-sight integration stages.
...
Organizing a code with a suitable modular structure that scales well in the modeling space is challenging.
...
For example, to implement a new species, the code must often be modified in many places: input handling for new parameters, new background equations, new thermodynamics equations, new perturbation equations, joining each of these stages, output handling, and so on.
...
As more species or gravitational theories are added, each module is intertwined with code from other physical components.
Even if they are deactivated by "if" statements at runtime, their mere presence in the source code increases its complexity and reduces its readability.
...
But this only forwards the problem.
...
They are also incompatible with each other unless they are merged into one code, which reintroduces the first problem.
...
For example, features such as nonlinear boosting and CMB lensing could be done by smaller and interoperating modular packages, but they often become part of \enquote{all-in-one} Boltzmann codes instead.
...
in the equations (e.g., \citealt{doranSpeedingCosmologicalBoltzmann2005,blasCosmicLinearAnisotropy2011a}):
\begin{itemize}
...
\end{itemize}
These involve switching from one set of equations to another when some control variable measuring the applicability of the approximation crosses a threshold.
This can change the variables in the ODE and require reinitializing the integration.
Approximations enable the use of explicit solvers and can improve both their speed and stability.
...
In machine learning, the same applies when training neural networks emulators for cosmological observables by minimizing a scalar loss function of parameters.
Some cosmologies are parameterized as boundary-value problems with the shooting method and use nonlinear root solvers such Newton's method, which require Jacobians.
...
but it is inherently symbolic and cannot differentiate arbitrary programs nested with control flow such as conditional statements and loops that depend on numerical values.
...
This can differentiate arbitrary programs, but it is approximate, introduces the step size, $\epsilon$, as a hyperparameter that must be tuned for both accuracy and stability. It is a brute-force approach that requires $O(n)$ evaluations ($2n$ using central differences) to compute the gradient of a function $f$ of $n$ variables.
...
This is numerically exact and free of precision parameters, while usually requiring fewer operations than finite differences, but it is perhaps less intuitive, harder to implement, and needs the source code of the program \eqref{eq:program} to interpret it in the nonstandard way \eqref{eq:program_derivative}.
...
In brief, it has a symbolic-numeric abstraction interface where users enter high-level symbolic equations and automatically compiles them to fast low-level numerical functions.
...
The end goal is to prioritize the modeler, who should be able to just write down their equations, while SymBoltz handles the modeling chores.

The code is written in the Julia programming language \citep{bezansonJuliaFreshApproach2017a}, which has a rich ecosystem of scientific packages and an aim to resolve the two-language problem.
The symbolic-numeric interface is built on ModelingToolkit.jl\footnote{\url{https://github.com/SciML/ModelingToolkit.jl}} by \cite{maModelingToolkitComposableGraph2022} and Symbolics.jl\footnote{\url{https://github.com/JuliaSymbolics/Symbolics.jl}} by \cite{gowdaHighperformanceSymbolicnumericsMultiple2022}.
The compiled functions are solved by implicit ODE integrators in OrdinaryDiffEq.jl\footnote{\url{https://github.com/SciML/OrdinaryDiffEq.jl/}} by \cite{rackauckasDifferentialEquationsjlPerformantFeatureRich2017} and linear algebra methods in LinearSolve.jl\footnote{\url{https://github.com/SciML/LinearSolve.jl}}.
Automatic differentiation works through ForwardDiff.jl\footnote{\url{https://github.com/JuliaDiff/ForwardDiff.jl}} by \cite{revelsForwardModeAutomaticDifferentiation2016}.
...
This paper is as of SymBoltz version 0.14.1.
...
Broadly speaking, the model defines what symbolic equations should be solved; the problem decides how it is solved; and the solution holds what was solved.
...
In our opinion, there are three properties that motivate some kind of symbolic abstraction layer such as this around the Einstein-Boltzmann equations.
...
While most codes are built as pipelines that follow the background, perturbations, and other stages,
SymBoltz is primarily structured around the physical components that make up the equations.
...
Any set of such submodels are joined into a full cosmological model, such as \LCDM{}.
...
This preprocessing does not slow the code at runtime; to the contrary, the symbolic equations give extra information such as the Jacobian that enhances speed and stability.
...
The generic symbolic framework can also be used to build reduced models with noninteracting radiation, matter and dark energy species.
...
In contrast, the monolithic layout of traditional codes scatter changes related to one submodel across different modules for input, the background, perturbations, output, and so on.
As more submodels are added, the code is increasingly intertwined and fragmented.
They are also not flexible enough to test simpler toy models, as baryons and photons are fundamentally hardcoded.
...
but does not focus on component-based modeling or automating tasks such as stage separation, for example.
...
The next sections explain how this is done.
Section \ref{sec:modifying} shows a concrete example and some of the symbolic features in action.

\subsubsection{Automatic numerical code generation}
\label{sec:codegen}

SymBoltz automatically compiles symbolic equations to numerical code ODEs,
\begin{equation}
...
The code generation handles tasks such as allocating indices for each ODE state, $u_i$, and performs optimizations such as common subexpression elimination (see Sect. \ref{sec:modifying}).
...
For example, we can consider the metric and gravity equations described in Appendices \ref{sec:metric} and \ref{sec:gravity} that have been sourced by some arbitrary $\rho(\tau)$, $\delta\rho(\tau,k),$ and $\Pi(\tau,k)$.
Here, $a(\tau)$ and $\Phi(\tau,k)$ are the only unknown (differential) variables that the ODE is solved for,
whereas we can \enquote{observe} $z(\tau)$, $\scrH(\tau)$, $H(\tau),$ and $\Psi(\tau,k)$ from the unknowns.
In addition, it is not always straightforward to express derivatives such as $\scrH^\prime$ or $\Psi^\prime$, but SymBoltz automatically expands them using the definitions of $\scrH$ or $\Psi$.
...
SymBoltz reads a full system of equations as input, such as that defined by the entirety of Appendix \ref{sec:implementation}, and separates variables into unknowns and observeds.
After solving the ODE for its unknowns, SymBoltz can automatically recompute any observed variable through its definition in terms of unknowns.
...
In other solvers, it might be necessary to look up observed expressions and recompute them manually.
...
In principle, the entire Einstein-Boltzmann equations (i.e., background and perturbations) can be integrated all at once.
However, they can be broken down into sequential computational stages that each depend only on those before it.
To alleviate stiffness in each stage, solve every equation only once (i.e., avoid recomputing the background for every perturbation mode), and to improve performance, all Boltzmann codes solve the system stage-by-stage and spline variables from one stage as input to the next.

To illustrate this case, we can again consider the general relativistic equations in Appendix \ref{sec:gravity} sourced by some known $\rho(\tau)$, $\delta\rho(\tau,k)$, and $\Pi(\tau,k)$.
...
Cubic Hermite splines are then constructed for all background unknowns (e.g., $a(\tau)$, $X_\Hy^+(\tau)$, $X_\He^+(\tau)$, $T_b(\tau)$ and $\kappa(\tau)$) and passed on to the perturbations.
This spline type is optimal for interpolating ODEs, as Hermite splines take both $\boldsymbol{u}(\tau)$ and $\boldsymbol{u}^\prime(\tau)$ into account for better accuracy, and $\boldsymbol{u}^\prime(\tau)$ is known analytically from $\boldsymbol{u}(\tau)$ through Eq. \eqref{eq:ode}.
% TODO: \LEt{ Please make sure to add "Eq." when including an equation number in the text for clarity.***}
In contrast, observed variables (e.g., $\scrH(\tau)$) are computed from the (splined) unknowns because their derivatives are not known directly, and splining them is less accurate.
This makes any background variable available in the perturbations \enquote{for free.}
All stages of the Einstein-Boltzmann equations are solved as if it is written as one common system of equations,
...
It just reflects the perturbative structure of the linear Einstein-Boltzmann equations, where each order depends only lower orders.
However, they can often be broken further down: most thermodynamics (recombination) models can be separated from the background, and some variables have integral solutions (e.g., the optical depth $\kappa(\tau) = \int_{\tau_0}^\tau \kappa^\prime(\bar\tau) \mathrm{d}\bar\tau$ and line-of-sight integration) that can be computed after solving the differential equations.
...
The solution interpolation is also incorporated into plotting recipes for visualization of any variable as a function of $\tau$ and $k$.

\subsubsection{Automatic Jacobian generation and sparsity detection}
\label{sec:jacobian}

Just as SymBoltz generates code for $\boldsymbol{f}$ in the ODE in Eq. \eqref{eq:ode}, it uses the same symbolic equations to generate its Jacobian, $\boldsymbol{J}$, with entries
\begin{equation}
...
SymBoltz generates them automatically to help the solver without distracting the modeler from focusing on the equations.
Automatic differentiation can also compute $\boldsymbol{J}$ from $\boldsymbol{f}$, but makes it harder to exploit its sparsity.
...
Without approximations, SymBoltz reuses the same sparsity structure throughout the integration, computes the nonzero $J_{ij}$ analytically, and stores them at a fixed index in the sparse $\boldsymbol{J}$.
This avoids converting $\boldsymbol{J}$ from dense to sparse form.
...
This is friendlier to the modeler, who must then simply provide a single set of equations, instead of deriving approximations, validating them, and dealing with related chores.
This approach is well-suited to a high-level equation-oriented symbolic interface.
...
where $\gamma$ is a constant, $h$ is the time step, and $\boldsymbol{J}$ is the ODE Jacobian in Eq. \eqref{eq:jacobian}.%
\footnote{
...
In return, implicit methods use more information about the ODE to take long and stable steps.
...
Bolt also defaults to this solver. DISCO-EB uses \texttt{Kvaerno5}, but we find it to be slower.

\item Rosenbrock methods \citep[e.g.,][]{langRosenbrockWannerMethodsConstruction2020} linearize $\boldsymbol{f} \approx \boldsymbol{J} \boldsymbol{u}$ and effectively replace Newton's method with one linear solving.
Their upside is being free from convergence on a nonlinear problem.
This suits the linear perturbation ODEs.
...
\end{itemize}
Performance of these methods is compared in Sect. \ref{sec:performance}.
...
This is wasteful, as $\boldsymbol{f} = \boldsymbol{J} \boldsymbol{u}$ for linear perturbations and computing the nonzeros of $\boldsymbol{J}$ takes fewer operations than $\boldsymbol{f}$!
...
It can also compute the ODE Jacobian, but we did this symbolically to get it analytically with its sparsity pattern.
...
The best way to illustrate the features in Sect. \ref{sec:features} is perhaps through some examples.
...
\begin{figure*}
    ...
    \caption{SymBoltz includes plotting recipes that make it trivial to visualize any symbolic variable or expression thereof from a solution of the Einstein-Boltzmann equations. This plot was made with one short line of code per subplot. Wavenumbers $k$ are in units of $H_0/c$.}
    \label{fig:output}
\end{figure*}
...
For example, \texttt{equations(M)} shows all equations; \texttt{equations(M.G)} only the gravitational ones; \texttt{M.g.a} refers to the scale factor variable $a(\tau)$ that \enquote{belongs} to the metric $g$; \texttt{M.b.$\Omega_0$} is the baryon density parameter $\Omega_{b0}$; and \texttt{parameters(M)} shows all parameters that can be set.
Everything displays with \LaTeX-compatibility in notebooks and transparently shows the equations being solved.

Second, the symbolic model is compiled to a numerical problem \texttt{prob} with parameters \texttt{p}.
This is an expensive step where the processing in Sect. \ref{sec:symbolicnumeric} happens: equations are checked for consistency and split into background and perturbation stages; observed and unknown variables are distinguished; fast ODE code is generated; background unknowns in the perturbations are replaced by splines; the Jacobian matrix is generated in numerical or analytical and dense or sparse form; and initial conditions are found.
...
For example, calling \texttt{sol(1.0, 2.0, g.$\Phi$+g.$\Psi$)} computes $\Phi+\Psi$ at $k = 1 \, H_0/c$ and $\tau = 2 \, H_0^{-1}$ by expanding it in terms of unknowns using the equations in Appendix \ref{sec:gravity}.
This interface is used to plot the evolution of several variables in Fig. \ref{fig:output} with little code.
...
Supposing we want to replace the cosmological constant $\Lambda$ with another dark energy model,
...
A $w_0 w_a \text{CDM}$ model is now built by replacing the cosmological constant species \texttt{$\Lambda$} in $\Lambda \text{CDM}$ by the $w_0 w_a$ species \texttt{X}:
\begin{codebox}
\begin{Verbatim}
...
The modification consists of writing down the equations almost verbatim,
and could hardly be more compact and to the point.
...
\item eliminate common subexpressions (e.g., $x\!=\!1+w$ and $y\!=\!1/x$);
...
In a purely numerical code, more manual work is required.
...
\begin{figure*}[ht!]
    ...
    \caption{Matter and CMB (TT, TE, and EE) power spectra computed by SymBoltz (SB; colored lines) compared to CLASS (CL; gray dashes) with relative errors $P_k^\text{SB}/P_k^\text{CL}-1$ and $C_l^\text{SB}/C_l^\text{CL}-1$ for the $w_0 w_a \text{CDM}$ model. CLASS uses the precision parameters given in Appendix \ref{sec:precision}.}
    \label{fig:spectra}
\end{figure*}
...
It is an equality for a Gaussian, for which the likelihood in Eq. \eqref{eq:likelihood_expansion} is exact.
Thus, inverting $\boldsymbol{F}(\boldsymbol{p}_0)$ gives the tightest possible parameter constraints.
...
\end{equation}
the Fisher matrix in Eq. \eqref{eq:fisher_general} becomes
\begin{equation}
...
Computing and inverting the Fisher matrix in Eq. \eqref{eq:fisher_special} forecasts the constraints in Fig. \ref{fig:forecast}.
...
Differentiable Fisher forecasts have also been demonstrated with an Eisenstein-Hu transfer function fit by \cite{campagneJAXCOSMOEndtoEndDifferentiable2023},
...
Finally, we show that SymBoltz can output differentiable results for gradient-based MCMC samplers, such as the No-U-Turn Sampler (NUTS).
It uses gradients to sample parameter space more efficiently than the \enquote{blind} Metropolis-Hastings algorithm.

We predicted apparent magnitudes $m(z) = M + \mu(z)$ of Type Ia supernovae at redshift, $z$, and fit to 1048 Pantheon observations $(z_i, m_i)$ by \cite{scolnicCompleteLightcurveSample2018}.
...
We define the likelihood $L$ by a multivariate normal distribution of the 1048 supernovae with covariance matrix $C$ given by \cite{scolnicCompleteLightcurveSample2018}.
To compute $L$, we use the probabilistic programming framework Turing.jl\footnote{\url{https://github.com/TuringLang/Turing.jl}} by \cite{fjeldeTuringjlGeneralPurposeProbabilistic2025} and specify a probabilistic model equivalent to,%
\begin{equation}
...
While these results are not groundbreaking, they show that the differentiable pipeline works.
We aim to speed it up with future work.
Differentiable Boltzmann codes have yet to show use with gradient-based MCMC samplers,
but \cite{campagneJAXCOSMOEndtoEndDifferentiable2023} did more advanced MCMCs with a differentiable background, for example.
...
\begin{figure}
    ...
    \caption{%
        ...
        Both codes solve a $w_0 w_a \text{CDM}$ model with equal parameters, $N=114$ equal wavenumbers $k_i$, multipole cutoff $\lmax=16$ for all species, default massive neutrino momentum sampling, and parallelization over $k$.
...
and \cite{leeRapidAccurateNumerical2025} sped up CLASS using integral equations for massive neutrinos.
...
These aspects are not yet as polished in SymBoltz as in CAMB and CLASS, so a detailed performance analysis for $C_l$ (beyond the runtimes in Table \ref{tab:timings}) is not meaningful now.
...
\item symbolic tensor manipulation for work with modified gravity.
\end{itemize}
...
as writing $\boldsymbol{u}(\tau,\boldsymbol{p})$ in the ODE in Eq. \eqref{eq:ode} and differentiating it for $\boldsymbol{v}_i = \partial \boldsymbol{u} / \partial p_i$ gives
...
Output power spectra agree with CLASS to $0.1\%$ with standard precision, so SymBoltz can be used fit to current data.
...
No differentiable Boltzmann code is capable of this as yet.

SymBoltz is easy to install from \url{https://github.com/hersle/SymBoltz.jl}.
Documentation\footnote{\url{https://hersle.github.io/SymBoltz.jl/}} is available and the code is tested with continuous integration to remain up-to-date (see Sect. \ref{sec:testing}).
...
The list is structured in the same way as SymBoltz', with one component per subsection.
Variables are in units in which \enquote{$G = c = H_0 = 1$}, and temperatures are in $\mathrm{K}$, unless otherwise stated.
...
This is perhaps the most common parameterization in the literature, and it is natural because most equations are autonomous with respect to $\tau$ (i.e., $f(\tau,\boldsymbol{u}) \rightarrow f(\boldsymbol{u})$, except some multipole truncation schemes that use $1/(k\tau)$, but these are unphysical).
...
Some important variables are the baryon temperature, $T_b$,
% TODO: \LEt{ When a symbol has the exact same meaning as the term preceding it and could be removed without changing the meaning of the sentence, it should be separated by parentheses or commas. Please check that my edits are accurate in this respect and make any further changes.***}
photon temperature, $T_\gamma$, mean molecular weight $\mu$, baryon sound speed, $c_s^2$, optical depth, $\kappa$, visibility function, $v$, and the free electron fraction, $X_\el$ (conventionally relative to Hydrogen, so $X_\el > 1$ in presence of Helium).
...
The baryon species is parameterized by the reduced density today, $\Omega_0 = \frac{8\pi}{3}\rho_0$, and the primordial Helium mass fraction, $Y_\He$.
...
This makes a clear distinction between the background with all zeroth-order equations of motion, and the perturbations with all first-order equations.
...
Unlike nonrelativistic particles, we must account for the direction $\cos\theta = \boldsymbol{p} \cdot \boldsymbol{k} / \lvert\boldsymbol{p}\rvert \lvert\boldsymbol{k}\rvert$ of their momenta, $\boldsymbol{p}$, relative to the Fourier wavenumber, $\boldsymbol{k}$.
...
The species is parameterized by its temperature today, $T_0$, which in turn sets the density parameters today, $\Omega_0 = \frac{\pi^2}{15} \frac{(k_B T_0)^4}{(\hbar c)^3} \frac{8\pi G}{3H_0^2}$ and $\rho_0 = \frac{8\pi}{3}\Omega_0$.
...
The species is parameterized by the effective number, $N_\text{eff}$, the reduced density, $\Omega_0 = \frac{8\pi}{3} \rho_0$, and temperature today, $T_0$.
...
In essence, the species we have looked at so far have Boltzmann equations where the momenta of their distribution function can be integrated out analytically in nonrelativistic and ultrarelativistic limits.
...
When integrated, these initial conditions are equivalent to adiabatic $\delta/(1+w)$, $\theta$, and $\sigma$, similarly to massless neutrinos.
Free parameters are the temperature today, $T_0$, mass of a single neutrino, $m$, and degeneracy factor, $N = \sum_{i=1}^N m_i / m$, for describing multiple neutrinos with equal mass.
...
Here the equation for $\psi_{i,l}^\prime$ hold for $2 \leq l \leq \lmax$ and the expressions $g_i = g(x_i)$ indexed by $i$ are evaluated with the momentum quadrature point $x = x_i$.
...
The comparison is found in SymBoltz' documentation.\footnote{\url{https://hersle.github.io/SymBoltz.jl/stable/comparison/}}
...
All parameter samples are found to integrate successfully.
