...
Fisher forecasting is powered by the Cramér-Rao bound $\big\lvert \smash{C_{ij}} \big\rvert \geq \big\lvert \smash{F^{-1}_{ij}} \big\rvert $ for the covariance $C_{ij}$ between parameters $\theta_i$ and $\theta_j$.
...
The derivatives $\partial C_l / \partial \theta_i$ are usually found with error-prone finite differences by carefully tuning the step.
...
This sets up a vector-to-vector function \texttt{Cl(θ)} that is differentiated by \texttt{ForwardDiff.jacobian}
