...
It suits the high-level equation-oriented symbolic interface.
...
$\boldsymbol{F}(\boldsymbol{u}_{n+1}) = \boldsymbol{u}_{n+1} - h \boldsymbol{f}(t_{n+1}, \boldsymbol{u}_{n+1}) - \boldsymbol{u}_n = \boldsymbol{0}$ at every step with Newton's method
using its Jacobian $\boldsymbol{W} = \boldsymbol{I} - h \boldsymbol{J}(t_{n+1},\boldsymbol{u}_{n+1})$.
}
...
To remain performant, implicit solvers need several optimizations.
They need efficient ways to compute not only $\boldsymbol{f}$, but also $\boldsymbol{J}$.
A common strategy is to reuse $\boldsymbol{W}$ and its LU-factorization over several steps.
Newton's method only needs an approximate $\boldsymbol{W}$ to iterate towards the root.
It is recomputed if it converges slowly, which means $\boldsymbol{J}$ is outdated or the step $h$ is changed by the adaptive solver.
This trades fewer LU-factorizations for more linear solve back-substitutions, which are only $O(n^2)$.
...
SymBoltz solves these issues by generating the analytical and sparse Jacobian, which is optimal in both speed and accuracy (see \cref{sec:jacobian}).

SymBoltz generates working code for all implicit solvers in OrdinaryDiffEq.jl\footnote{\url{https://github.com/SciML/OrdinaryDiffEq.jl/}} and both dense and sparse matrix methods in LinearSolve.jl\footnote{\url{https://github.com/SciML/LinearSolve.jl}}.
We can then test several methods by changing one line of code.
This is useful, as performance and stability of implicit solvers and linear algebra can be highly problem-dependent.
...
We find the following ODE solvers to work:
...
\begin{itemize}
\item \textbf{BDF methods:}
Backward Differentiation Formula methods like \texttt{FBDF} and \texttt{QNDF} \citep{shampineMATLABODESuite1997} are 
multi-step methods that use information from several past steps at every step.
Without \enquote{stages} in single-stepping Runge-Kutta methods,
they solve just one implicit equation per step, scale well for larger ODEs and reuse Jacobians aggressively.
They have variable order of 1-5, but their most desirable L-stability property only holds up to 2nd order.
This makes them take very small steps and not utilize their high order in stiff regimes.
...
\item \textbf{ESDIRK methods:}
Explicit Singly-Diagonal Implicit Runge-Kutta methods \citep[e.g.][]{jorgensenFamilyESDIRKIntegration2018} like \texttt{KenCarp4} \citep{kennedyAdditiveRungeKutta2003} are a class of implicit Runge-Kutta methods designed to preserve most stability of Fully Implicit (FIRK) methods with less computational cost.
An $s$-stage FIRK method has excellent stability, but a dense Butcher tableau $a_{ij}$ and must solve one big system of $n \times s$ equations at every step.
D in ESDIRK means that $a_{ij}$ only has lower triangular and diagonal nonzeros, so the $n \times s$ system decouples into $s$ systems of $n$ equations that are easier to solve sequentially.
S means that all (nonzero) diagonal entries $a_{ii} = \gamma$ are equal, so all stages use the same $\boldsymbol{W}$-matrix that can be reused over steps as in BDF methods.
E means that $a_{11} = 0$, so the first stage is explicit and cheap.
Unlike BDF methods, these methods remain L-stable without losing order in stiff regimes.
We find the 4th order \texttt{KenCarp4} method to perform well on the Einstein-Boltzmann equations with much fewer time steps.
Bolt.jl also defaults to this solver, and DISCO-EB and PyCosmo use \texttt{Kvaerno5} and \texttt{TRBDF2} from the same family.

\item \textbf{Rosenbrock methods:} Rosenbrock methods \citep[e.g.][]{langRosenbrockWannerMethodsConstruction2020} like \texttt{Rodas5P} \citep{steinebachConstructionRosenbrockWanner2023} linearize $\boldsymbol{f} \approx \boldsymbol{J} \boldsymbol{u}$ and write out only one iteration of Newton's method.
Being free from nonlinear convergence is a major upside and also suitable for the linear perturbation ODEs.
The downside is that they can require a new $\boldsymbol{J}$ at every single step, but this is largely eliminated with a generated analytical Jacobian.
A subclass called Rosenbrock-W methods can in principle reuse $\boldsymbol{J}$ over time steps, but this strategy is not yet implemented in OrdinaryDiffEq.jl.
We find this to perform best on both the background and perturbations in terms of both stability and stiffness, and have not found it used by other Boltzmann codes.
\end{itemize}
\Cref{sec:performance} gives a performance comparison of these methods.

In contrast, traditional codes use approximations to reduce stiffness and improve performance by evolving fewer ODE variables when possible.
Previous works have found only marginal speedups from TCA and UFA compared to use of implicit solvers,
...
But approximation-based solvers are more complicated and must account for the structure of the equations to change.
This is not without overhead for implicit solvers and sparse Jacobian structures.
Approximation-free solvers can challenge this by optimizing around unchanged structure.
Furthermore, non-differentiable codes compute $\boldsymbol{J}$ with finite differences with $O(n)$ evaluations of $\boldsymbol{f}$.
This is extremely wasteful, as $\boldsymbol{f} = \boldsymbol{J} \boldsymbol{u}$ for the linear perturbations and computing the nonzeros of $\boldsymbol{J}$ in fact requires fewer operations than $\boldsymbol{f}$!
Approximations also put more load on the modeler to derive, implement and validate them,
and this must generally be repeated for modifications that easily reintroduce stiffness or invalidate the approximations.
...
It can compute derivatives of any output quantity, like ODE variables and derived spectra, with respect to any input parameters, like the Hubble parameter $h$ or cold dark matter density parameter $\Omega_{c0}$.
For example, it can be used to learn the output's sensitivity to the input, perform Fisher forecasts or guide the shooting method when parametrizing cosmological models with integrated variables.
It can also compute the ODE Jacobian, but we do this symbolically to get it analytically and find its sparsity pattern.

Currently SymBoltz is well-tested with forward-mode dual numbers through ForwardDiff.jl\footnote{\url{https://github.com/JuliaDiff/ForwardDiff.jl}}.
This is most appropriate for applications with more outputs than inputs, such as analyzing sensitivity of $\mathcal{O}(100)$ values of $P(k)$ or $C_l$ as a function of $\mathcal{O}(10)$ cosmological parameters.
Support for reverse-mode would be ideal when there are fewer outputs.
This is very attractive for like MCMCs or training neural network emulators, where prediction-data errors are compressed to a scalar loss value.
Cheap reverse-mode gradients could accelerate sampling of $O(100)$-dimensional parameter spaces with more experimental nuisance parameters in next-generation surveys.
Emulators have become popular \enquote{workarounds} to do this cheaply and with existing theory codes, instead of adapting codes to be differentiable.
Unfortunately, reverse-mode is left for future work.
...
\begin{table}[b]
\caption{\label{tab:timings}Wall time spent by SymBoltz and CLASS to compute the power spectra $P(k)$ and $C_l$ (TT+TE+EE) in \cref{fig:spectra}.}
...
This is a common trick in Boltzmann solvers to integrate fewer perturbation modes.
Output spectra and their computation time are shown in \cref{fig:spectra,tab:timings}.

SymBoltz and CLASS agree to around $0.1\%$ or better.
The $P(k)$ agree to $0.1\%$ for all $k$ and $0.01\%$ for linear $k$.
The $C_l$ agree to $0.1\%$ for most $l$, but slightly worse for very low $l$ (dominated by cosmic variance) and very large $l$ in $C_l^\text{TE}$.
This is the level of accuracy needed to fit current-generation data \TODO{ref}.

SymBoltz computes $P(k)$ quickly, but the extra steps needed for $C_l$ are not yet as optimized as they are in CLASS.
A more thorough comparison of performance vs. precision for computing $P(k)$ with SymBoltz vs. CLASS is given in \cref{sec:performance}.
