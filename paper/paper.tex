...
\begin{equation}
    \frac{\mathrm{d}\mathbf{u}}{\mathrm{d} \tau} = \mathbf{f}(\tau,\mathbf{u},\mathbf{p}).
    \label{eq:ode}
\end{equation}
...
as writing $\mathbf{u}(\tau,\boldsymbol{p})$ in the ODE \eqref{eq:ode} and differentiating it for $\boldsymbol{v}_i = \partial \boldsymbol{u} / \partial p_i$ gives
$\mathrm{d}\mathbf{v}_i/\mathrm{d}\tau = \mathbf{J} \mathbf{v}_i + \partial \mathbf{f} / \partial p_i$ and is hard to construct by hand.
...
A recent example of this is DISCO-DJ's impressive end-to-end modeling from linear perturbations to non-linear structure formation by coupling differentiable Boltzmann and $N$-body codes \citep{hahnDISCODJDifferentiableEinsteinBoltzmann2024,listDISCODJIIDifferentiable2025}.
