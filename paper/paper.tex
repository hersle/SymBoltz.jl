...
Minimizing friction in the modeling process encourages relaxing model-dependent approximations, creating user-friendly interfaces and structuring codes with modular components that are easy to replace.
...
SymBoltz is a new Boltzmann solver that aims to meet these needs, featuring a convenient symbolic-numeric interface, approximation-freeness and differentiability.
...
\Cref{sec:intro} motivates SymBoltz by reviewing the historical development of Boltzmann codes and methods for computing derivatives.
...
\Cref{sec:implementation,sec:precision,sec:testing} list equations, implementation details, comparison settings and testing methods.

\section{History and motivation}
\label{sec:intro}

\cite{peeblesPrimevalAdiabaticPerturbation1970} were first to numerically integrate a comprehensive set of linear Einstein-Boltzmann equations.
...
to solve these ordinary differential equations (ODEs) ${\mathrm{d}\boldsymbol{u}}/{\mathrm{d} \tau} = \boldsymbol{f}(\tau,\boldsymbol{u},\boldsymbol{p})$ for some initial conditions $\boldsymbol{u}(\tau_0)$ and parameters $\boldsymbol{p}$, including several perturbation wavenumbers, and compute desired quantities from their solution, like power spectra.
...
Fifth, the perturbations must be solved for many different wavenumbers $k$, and tradeoffs between performance and precision must be made.
...
Traditional codes have nonexistent or thin abstraction layers.
...
Some features are really just post-processing of the ODE solutions, but are appended to the pipeline even if peripheral to the core task of an Einstein-Boltzmann solver -- solving the Einstein-Boltzmann equations.
Features like non-linear boosting and CMB lensing could be made as smaller and interoperating modular packages, but instead become part of \enquote{all-in-one} Boltzmann codes.
...
In machine learning, the same applies when training neural networks emulators for cosmological observables as functions of parameters by minimizing a scalar loss.
...
The former specializes every function on a particular \enquote{dual number} that computes both the value and derivative of the function \cite[e.g.][]{revelsForwardModeAutomaticDifferentiation2016}.
...
\textbf{This paper is as of SymBoltz version 0.14.1. Please see the package documentation for definitive up-to-date information.}
...
A general ODE \eqref{eq:ode} admits two types of variables: \textit{unknowns} $\boldsymbol{u}(\tau)$ are integrated with respect to time, while \textit{observed} variables are any functions of the unknowns.
...
To alleviate stiffness in each stage, separate concerns by solving every equation only once (e.g. avoid recomputing the background for every perturbation mode) and to improve performance, all Boltzmann codes solve the system stage-by-stage and spline variables from one stage as input to the next.
...
SymBoltz uses the same stage separation strategy, but automates it with its knowledge of the symbolic equations.
First, all symbolic equations are split into background and perturbation stages.
...
In the future, SymBoltz' background-perturbations separation can be extended to automatically split all smaller stages by symbolically inspecting the dependencies between variables in the full equations.
...
This is hard to specify manually, and numerical solvers cannot distinguish false local zeros (for some input) from true global zeros (for all inputs).
...
Easy, accurate, fast and sparse Jacobians are crucial for SymBoltz to remain performant without approximations!
...
\item \textbf{Rosenbrock methods:} Rosenbrock methods \citep[e.g.][]{langRosenbrockWannerMethodsConstruction2020} linearize $\boldsymbol{f} \approx \boldsymbol{J} \boldsymbol{u}$ and effectively replace Newton's method with one linear solve.
...
This simplifies the equations and remains performant due to high-order implicit solvers with fast generated code for ODEs and analytical and sparse Jacobians.
It is differentiable, so one can get accurate derivatives of any output quantity with respect to any input parameter.
Output power spectra agrees with CLASS to $0.1\%$ and can be used fit to current data.
...
This is perhaps the most common parametrization in the literature, and is natural because most equations are autonomous with respect to $\tau$ (i.e. $f(\tau,\boldsymbol{u}) \rightarrow f(\boldsymbol{u})$, except some multipole truncation schemes that use $1/(k\tau)$, but these are unphysical).
