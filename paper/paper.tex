...
Implicit ODE solvers take more expensive but longer steps than explicit methods on stiff problems.
Every time step solves a possibly nonlinear system of equations, which involves the Jacobian of the system and in turn the ODE Jacobian \eqref{eq:jacobian}.
This can become a bottleneck as the number of equations increases (e.g. with larger $\lmax$).
Automatic, analytic and efficient computation of the Jacobian in sparse form significantly mitigates this.

SymBoltz supports all implicit solvers in the OrdinaryDiffEq.jl package.
These have different performance and stability characteristics.
In particular, we find that the \texttt{Rodas5P} solver \TODO{cite} performs well on both the background and perturbations of the stiff Einstein-Boltzmann equations.
\TODO{app}

In the long run, SymBoltz could also implement approximation schemes as a secondary and optional way to maximize speed.
But the goal is to keep the code primarily approximation-free.
...
The work is dominated by the perturbations, so this is a relevant test of SymBoltz' approximation-free treatment of perturbations.
...
\TODO{Rosenbrock vs. BDF vs. SDIRK}
\TODO{emphasize NDF in CLASS, TRBDF2 in PyCosmo, Kvaerno5 in DISCO-DJ}
\TODO{first comprehensive solver comparison}
...
As a counterweight to the major traditional codes CAMB and CLASS in which some approximations cannot be disabled, SymBoltz' primary mission will be to solve full stiff equations and keep approximations optional and secondary.
