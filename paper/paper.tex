...
\midrule
SymBoltz (no approximations) & 0.5 s & 7.3 s \\
CLASS (only tight-coupling approximation) & 8.7 s & 15.9 s \\
CLASS (all approximations) & 0.7 s & 2.9 s \\
\bottomrule
\end{tabularx}
\end{table}
...
\Cref{tab:timings} shows the time to compute the spectra.
We note that SymBoltz computes $P(k)$ quite fast, but the extra steps needed for $C_l$ it is not yet as optimized as in CLASS.
\Cref{sec:performance} analyzes performance in greater detail.
