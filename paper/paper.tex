...
To overcome this, it is helpful to automatically generate low-level code for the ODE that is optimal in speed, accuracy and stability.
This can be extended to generate the ODE Jacobian, which is nontrivial to derive manually but crucial for implicit solvers to solve stiff ODEs.
Modelers should not have to worry about this.

Third, the equations are often modified because the true cosmological model is unknown.
This can be made easier by organizing the equations in a way that reflects the physical structure of the model, so it is easy to replace a subset of equations with another without duplicating the entire model.
...
While other codes are built as pipelines to follow the background, perturbations and other stages of the equations,
SymBoltz is primarily structured around the physical components that it consists of.
Related variables and equations are grouped into distinct components for the metric, gravitational theory, photons, baryons, dark matter, dark energy, neutrinos and other species.
...
Interactions (e.g. Compton scattering or sourcing of gravity) are equations between several components.
...
As more and more submodels are added, their code is increasingly intertwined and the fragmentation is amplified.
...
The powerful symbolic interface makes it very compact:
...
It gives maximal speed, stability and convenience from minimal user input.
