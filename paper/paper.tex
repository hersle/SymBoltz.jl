...
\begin{table}[b]
\caption{\label{tab:timings}Wall time spent by SymBoltz and CLASS to compute the power spectra $P(k)$ and $C_l$ (TT+TE+EE) in \cref{fig:spectra} with standard precision.}
...
\end{Verbatim}
\end{codebox}
Calling \texttt{spectrum\_matter} and \texttt{spectrum\_cmb} with \texttt{prob} automatically selects a coarse $k$-grid to solve the perturbations for and interpolates to a finer $k$-grid.
This is a common trick in Boltzmann solvers to integrate fewer $k$-modes.
Both functions take in all precision parameters that affect the computation as optional keyword arguments.
Output spectra and their computation time with standard precision settings are shown in \cref{fig:spectra,tab:timings}.

We see that SymBoltz and CLASS agree to around $0.1\%$ or better.
The $P(k)$ agree to $0.1\%$ for all $k$ and $0.01\%$ for linear $k$.
The $C_l$ agree to $0.1\%$ for most $l$, but slightly worse for very low $l$ (where cosmic variance is also larger) and very large $l$ in $C_l^\text{TE}$.
This level of accuracy is sufficient for most current data \citep[e.g.][]{bollietHighaccuracyEmulatorsObservables2023}.
It is around the level of CAMB vs. CLASS run with standard precision,
although their agreement can be pushed to $0.01\%$ using high precision \cite{lesgourguesCosmicLinearAnisotropy2011c}.
