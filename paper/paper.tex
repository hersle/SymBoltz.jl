...
Cosmological models are built from replaceable physical components in a way that scales well in model space, or alternatively written as one simple and compact system of equations.
...
Implicit solvers integrate the full stiff equations at all times, which greatly simplifies the code.
Performance remains as good as in approximation-based codes due to use of high-order implicit methods that take long time steps, optimal handling of the Jacobian and fast sparse matrix methods.
...
The features form a synergy that reinforces the design of the code.
Results agree with established codes like CLASS and CAMB to $0.1\%$ with standard precision.
...
SymBoltz is available at \url{https://github.com/hersle/SymBoltz.jl} with single-command installation and extensive documentation, and welcomes all contributions.
...
In high dimensions, modern Markov chain Monte Carlo methods like Hamiltonian Monte Carlo and the No-U-Turn Sampler \citep{hoffmanNoUTurnSamplerAdaptively2011} outperform the traditional Metropolis-Hastings algorithm \citep{hastingsMonteCarloSampling1970}.
...
For example, it predicts cosmic microwave background (CMB), baryon acoustic oscillation (BAO) or supernova (SN) observations, or generates initial conditions for non-linear $N$-body simulations of large-scale structure.

This article is structured as follows.
\Cref{sec:intro} revisits the historical development of Boltzmann codes, introduces methods for computing derivatives and motivates SymBoltz.
...
\Cref{sec:performance} compares performance to one existing code.
\Cref{sec:futurework} describes possibilities for future work.
...
Codes are usually tightly adapted to the pipeline-like computational structure of the problem (e.g. input $\rightarrow$ background $\rightarrow$ thermodynamics $\rightarrow$ perturbations $\rightarrow \ldots \rightarrow$ output in section 3.3 in \cite{lesgourguesCosmicLinearAnisotropy2011}).
This makes sense for programmers and computers, but not always for modelers.

Traditional codes have nonexistent or thin abstraction layers around them.
To modify them, users must work directly in the low-level numerical interface and have a good understanding of the complex internal structure of the code.
For example, to implement a new species, one must generally modify the code in many places: input handling for new parameters, new background equations, new thermodynamics equations, new perturbation equations, joining each of these stages, output handling, and possibly more.
...
As more species or gravitational theories are added, each module becomes intertwined with code from many physical components.
Even if they are deactivated by if statements at runtime, their mere presence in the source code reduces its readability.
In turn, the code can grow into a complex and convoluted system.
...
Some features are really just post-processing of the ODE solutions, but appended to the pipeline even if peripheral to the core task of an Einstein-Boltzmann solver -- solving the Einstein-Boltzmann equations.
Features like non-linear boosting and CMB lensing could be made as smaller modular packages, but instead become part of overwhelming \enquote{all-in-one} Boltzmann codes.

These patterns lead to big monolithic Boltzmann solvers that scale poorly in model space, and whose complexity grows to incorporate features beyond their original core scope.
...
The Einstein-Boltzmann equations are infamously stiff.
This is a property of differential equations that means their numerical solution is unstable and requires tiny step sizes with standard explicit Runge-Kutta methods, making them hard to solve.
...
in the equations (e.g. \cite{doranSpeedingCosmologicalBoltzmann2005,blasCosmicLinearAnisotropy2011a,lewisCAMBNotes2025}):
\begin{itemize}
...
Approximations can help both stability and performance and allow the use of explicit solvers.
...
This process may have to be repeated when the model is modified.
The numerics also become more complicated: timeseries from each ODE solution must be stitched together, each separate ODE system can use different tolerances, ODE integrators must be reinitialized, and so on.

%For a long time, approximation schemes was the only way considered to handle the stiffness in the Einstein-Boltzmann equations \citep{maCosmologicalPerturbationTheory1995,lesgourguesCosmicLinearAnisotropy2011,lewisCAMBNotes2025}.
Another way to integrate stiff equations is to use appropriate implicit solvers.
%They are well-known to be appropriate in such situations.
\cite{refregierPyCosmoIntegratedCosmological2017a} first handled the full stiff Einstein-Boltzmann equations with an implicit integrator in PyCosmo, followed by Bolt.jl and DISCO-EB.
CLASS also has an implicit solver, but does not permit disabling all approximations \citep{blasCosmicLinearAnisotropy2011a}.
Historically, implicit solvers may have been underutilized because they are harder to implement than explicit solvers and have a reputation for being slower than explicit solvers,
and due to iterative evolution from initial Boltzmann codes centered around approximations.
...
\begin{figure*}
    ...
    \end{tikzpicture}
    \caption{SymBoltz represents cosmological models with symbolic equations grouped by physical components like the metric, gravity and particle species. This is compiled to a numerical problem that splits the equations into background and perturbation stages and generates fast code for ODEs and their Jacobians. The problem is then solved and the result stored in a solution object that gives convenient access to any variable in the model.}
...
SymBoltz is written in the Julia programming language \citep{bezansonJuliaFreshApproach2017a}, with a rich ecosystem of scientific packages and an aim to resolve the two-language problem.
...
\textbf{This paper is as of SymBoltz version 0.14.0. Please refer to the package documentation for definitive up-to-date information.}
...
With knowledge of the symbolic equations, SymBoltz can analyze their structure programmatically to ease model specification, automate manual chores related to solving them and generate fast and stable code that avoids approximations.
...
To overcome this, it is helpful to automatically generate ODE code that is optimal in speed, accuracy and stability, and includes nontrivial constructs like the ODE Jacobian that implicit solvers may take advantage of.
...
While other codes are pipelines that follow the background, perturbations and other stages of the equations,
...
SymBoltz then takes a full cosmological model, separates it into stages and generates numerical code to solve each stage with ODE methods.
This preprocessing does not slow the code at runtime; to the contrary the symbolic equations give extra information that is used to enhance speed and stability.
...
%It makes it easier to assemble both reduced and extended cosmological models.
...
When exploring modified gravity theories, it can be helpful to test such toy models to understand how gravity responds to pure fluids without coupling to baryons and photons. % e.g. https://github.com/lesgourg/class_public/issues/246

In contrast, the monolithic layout of other codes scatter changes related to one submodel across different modules for input/output, the background, perturbations and so on.
As more and more submodels are added, this code is increasingly intertwined and the fragmentation is amplified.
%Adding new submodels then effectively duplicates one big monolithic model and leads to many forked versions of a Boltzmann code with different extensions that are incompatible with each other.
...
%\TODO{e.g. implementing many different gravity models that do not have the same limit}

As an alternative to the component-based modeling,
...
This makes it trivial to change absolutely anything in the equations, but sacrifices modularity.
The symbolic interface makes it very compact:
SymBoltz defines the full \LCDM{} model (with GR, baryons, RECFAST recombination, photons, cosmological constant, massive and massless neutrinos) in just 277 straightforward lines of code, while the equivalent code to browse in CLASS is spread over 10 files with 27721 lines.%
...
CLASS' greater number of features does not make up for the $100\times$ simplification.

SymBoltz' powerful symbolic-numeric interface aims to make it as easy as possible to solve and modify the Einstein-Boltzmann equations.
Its goal is to maximize speed, stability and convenience with minimal user input.
The next sections go into detail on how these overarching features are implemented.
...
If necessary, one can escape the standard code generation and call arbitrary numerical functions, for example if one wants to solve a nonlinear equation for the minimum of a potential or interpolate tabulated data.
The code generation handles chores like allocating indices for each ODE state $u_i$, and performs optimizations such as common subexpression elimination.
...
A general ODE \eqref{eq:ode} admits two types of variables: \textit{unknowns} $\mathbf{u}(\tau)$ are integrated with respect to time, while \textit{observed} variables are any functions of the unknowns.
...
Here $a(\tau)$ and $\Phi(\tau,k)$ are the only unknown (differential) variables that the ODE solver solve for,
while one can \enquote{observe} $z(\tau)$, $\scrH(\tau)$, $H(\tau)$ and $\Psi(\tau,k)$ from the unknowns.
In addition, observed derivatives like $\scrH^\prime$ or $\Psi^\prime$ are not always trivial to compute, but SymBoltz automatically expands them using the definitions of $\scrH$ or $\Psi$.
Of course, one can eliminate all observeds by explicitly inserting them into the equations for the unknowns.
But this kills readability, as observed variables are helpful intermediate definitions that break up the system, and one may want to extract them from the solution as well.
...
The bottom line is that users can easily refer to and obtain \textit{any} variable anywhere just by referring to it, whether it is unknown or observed.
In other solvers one must look up the desired expression for observed variables and recompute them manually.
This is tedious, error-prone and tempts users to neglect parts of expressions (e.g. approximating $\Psi \approx \Phi$ without anisotropic stress).

\subsubsection{Automatic stage separation and splining of unknowns}
\label{sec:splining}

In principle, one can solve the Einstein-Boltzmann equations by integrating the entire system (e.g. background and perturbations) at once.
...
First, the full symbolic equations are split into background and perturbation systems.
Cubic Hermite splines are then constructed for all background unknowns (like $a(\tau)$) and passed on to the perturbations.
This type of splines is optimal for interpolating ODEs, as Hermite splines increase accuracy by taking both $\boldsymbol{u}(\tau)$ and $\boldsymbol{u}^\prime(\tau)$ into account, and $\boldsymbol{u}^\prime(\tau)$ is known analytically by definition \eqref{eq:ode}.
In contrast, observed variables (like $\scrH(\tau)$) are computed from the (splined) unknowns because their derivatives are not known directly.
Splining them directly is less accurate.
Modelers can simply write down a new background variable and access it in the perturbations for free.
Any variable can be accessed anywhere as if solving the entire set of equations at once, while splining and stage separation is still used under the hood.
...
SymBoltz' background-perturbations separation could be extended to automatically split all such smaller stages by symbolically inspecting the dependency between variables of the full Einstein-Boltzmann system.
...
All the results are stored in a solution object.
This solution object can be conveniently queried for any variable or symbolic expression
\begin{equation}
    S(\boldsymbol{u}(\tau, k)).
\end{equation}
This looks up the background solution if $S$ is a background variable, or the perturbation solutions if it is perturbative.
It interpolates from the times stored by the ODE solver to the requested $\tau$ using the solver's dense interpolation method.
If $S$ is perturbative, it also interpolates from the integrated perturbation $k$-modes to the requested $k$.
...
The solution interpolation is also incorporated into convenient plot recipes for trivial visualization of any variable as a function of $\tau$ and $k$.

\subsubsection{Automatic Jacobian generation and sparsity detection}
\label{sec:jacobian}

Just like SymBoltz generates code for $\boldsymbol{f}$ in the ODE \eqref{eq:ode}, it uses the same symbolic equations to generate its Jacobian $\boldsymbol{J}$ with values
\begin{equation}
...
Analytical Jacobians are as fast and stable as possible, and automatic generation of it lets the modeler enjoy its benefits while focusing only on the high-level equations.
...
Manual and numerical approaches to this are again neither as friendly nor robust.
Without approximations, it then reuses the sparsity structure throughout the integration, computes the nonzero $J_{ij}$ analytically and stores them directly in the sparse $\boldsymbol{J}$ with no extra cost from looking up sparse matrix indices.

Easy, accurate and fast Jacobians are very important for SymBoltz to remain fast without approximations!
\Cref{sec:performance} discusses implications for performance in more technical detail and relates the Jacobian handling to that in other Boltzmann solvers.
...
This pairs well with high-level equation-oriented symbolic interface.
...
Using Newton's method, it iteratively solves linear systems $\boldsymbol{W} \boldsymbol{u} = \boldsymbol{b}$
...
To remain fast, implicit solvers need several optimizations.
They must compute not only $\boldsymbol{f}$, but also $\boldsymbol{J}$ efficiently.
A common strategy is to reuse the LU-factorized $\boldsymbol{W}$ over several steps.
...
This trades fewer LU-factorizations and $\boldsymbol{J}$ evaluations for more linear solve back-substitutions, which only cost $O(n^2)$.
The next trick is to use sparse matrices if $\boldsymbol{W}$ contains many zeros, as is the case for large perturbation systems.
SymBoltz handles this by generating the analytical and sparse Jacobian in the fastest and most accurate way (see \cref{sec:jacobian}).
...
We can test different methods, which is useful as performance and stability of implicit solvers and linear algebra operations are problem-dependent.
...
% https://scicomp.stackexchange.com/questions/31380/what-is-the-state-of-the-art-in-solving-stiff-initial-value-problems
% https://www.stochasticlifestyle.com/differences-between-methods-for-solving-stiff-odes/

\begin{itemize}
\item
%\textbf{BDF methods:}
\textbf{Backward Differentiation Formula (BDF) methods}
like \texttt{FBDF} and \texttt{QNDF} \citep{shampineMATLABODESuite1997} are 
multi-step methods that use several points in the past to find the next step.
They do not have \enquote{stages} as in Runge-Kutta methods and solve just one implicit equation per step,
so they scale best for larger ODEs and can reuse Jacobians aggressively.
They have variable order of 1-5, but their most desirable L-stability property only holds up to 2nd order.
This makes them take very small steps in stiff regimes without utilizing their highest orders.
%They reuse Jacobians very aggressively by keeping the time step constant.
%but we do not find this to be the case with sparse matrices and only $O(100)$ equations per perturbation mode.
%Being multistep methods, they do not have \enquote{stages} but compute the next time point in one point.
% TODO: multi-step structure is not optimal for events like approximation switching
We find them to be the slowest.
CLASS' \texttt{ndf15} evolver belongs to this family \citep{lesgourguesCosmicLinearAnisotropy2011a}.

\item
%\textbf{ESDIRK methods:}
\textbf{Explicit Singly-Diagonal Implicit Runge-Kutta (ESDIRK) methods} \citep[e.g.][]{jorgensenFamilyESDIRKIntegration2018} are a class of implicit Runge-Kutta methods designed to preserve most stability properties of Fully Implicit (FIRK) methods at a cheaper computational cost.
...
Bolt.jl also defaults to this solver, and DISCO-EB and PyCosmo use \texttt{Kvaerno5} and \texttt{TRBDF2} from the same family, but we find them slower.

\item \textbf{Rosenbrock methods:} Rosenbrock methods \citep[e.g.][]{langRosenbrockWannerMethodsConstruction2020} linearize $\boldsymbol{f} \approx \boldsymbol{J} \boldsymbol{u}$ and replace Newton's method with one linear solve.
Their major upside is being free from convergence on a nonlinear problem.
This suits the perturbation ODEs that are already linear.
The downside is that they can require a new $\boldsymbol{J}$ at every step, but this is eliminated by generating it analytically.
A subclass called Rosenbrock-W methods can in principle reuse Jacobians, but this strategy is not yet used by OrdinaryDiffEq.jl.
...
\end{itemize}
\Cref{sec:performance} gives a performance comparison of these methods.

In contrast, traditional codes use approximations to reduce stiffness and increase performance by evolving fewer ODE variables when possible.
Earlier works find only marginal speedups from the TCA and UFA compared to using implicit solvers,
while the RSA can significantly speed up models with high $\lmax$ \citep{lesgourguesCosmicLinearAnisotropy2011a,moserSymbolicImplementationExtensions2022,hahnDISCODJDifferentiableEinsteinBoltzmann2024}.
But approximation-based codes must account for the structure of the equations to change, which adds overhead to implicit solvers and restructuring sparse Jacobians.
Approximation-free solvers optimize around unchanged structure to challenge this.
Non-differentiable codes approximate $\boldsymbol{J}$ with finite differences and $O(n)$ evaluations of $\boldsymbol{f}$.
This is very wasteful, as $\boldsymbol{f} = \boldsymbol{J} \boldsymbol{u}$ for linear perturbations and computing the nonzeros of $\boldsymbol{J}$ in fact requires fewer operations than $\boldsymbol{f}$!
Codes with a symbolic structure can take advantage of such properties.
...
For example, it can be used to learn the output's sensitivity to the input, perform Fisher forecasts or compute the Jacobian in the shooting method when parametrizing models with integrated variables.
It can also compute the ODE Jacobian, but we do this symbolically to get it analytically with its sparsity pattern.

Currently, SymBoltz works with forward-mode dual numbers through ForwardDiff.jl\footnote{\url{https://github.com/JuliaDiff/ForwardDiff.jl}}.
This is appropriate for applications with more outputs than inputs, like differentiating $\mathcal{O}(100)$ values of $P(k)$ or $C_l$ as functions of $\mathcal{O}(10)$ parameters.
...
Cheap reverse-mode gradients could accelerate sampling of $O(100)$-dimensional parameter spaces with more nuisance parameters from upcoming surveys.
Emulators have become a popular tool to do this with existing theory codes without rewriting them to be differentiable.
However, reverse-mode is left for future work.

\section{Examples}
\label{sec:examples}

The best way to illustrate the features in \cref{sec:features} is perhaps through some examples.
The full code is as of SymBoltz version 0.14.0 and available in a notebook in the project repository.
...
For example, \texttt{equations(M)} shows all model equations; \texttt{equations(M.G)} only the gravitational ones; \texttt{M.g.a} refers to the scale factor variable $a(\tau)$ that \enquote{belongs} to the metric $g$; \texttt{M.b.Ω₀} is the baryon density parameter $\Omega_{b0}$; and \texttt{parameters(M)} lists parameters of the model that the user may set.
Everything displays with \LaTeX-compatibility in notebooks for interactive use.

Second, the symbolic model is compiled to a numerical problem \texttt{prob} with parameters \texttt{pars}.
This is an expensive operation where the processing in \cref{sec:symbolicnumeric} happens: equations are checked for consistency and split into background and perturbation stages; observed and unknown variables are distinguished; fast ODE code is generated; background unknowns in the perturbations are replaced by splines; the Jacobian matrix is generated in numerical/analytical and dense/sparse form; and initial values are computed.
One can customize whether stages should be separated, how the Jacobian is computed, and so on.
...
This convenient interface was used to easily plot the evolution of several variables in \cref{fig:output}.

Other Boltzmann solvers save only a fixed set of variables, such as only the unknowns.
...
push!(pars, X.w₀ => -0.9, X.wₐ => 0.2, X.cₛ² => 1.0)
...
\item spline $\rho(\tau)$ in the perturbations (had we used \eqref{eq:w0wadiff} over \eqref{eq:w0waanal}),
\item source gravity with energy-momentum contributions,
\item assign $\Omega_0 = 1 - \sum_{s \neq {w_0 w_a}} \Omega_{s0}$ (if gravity is GR),
\item eliminate common subexpressions like $x=1+w$ and $y = 1/x$, % and multiply by $y$ instead of dividing by $x$,
\item generate ODE functions for $\delta^\prime$ and $\theta^\prime$,
\item generate sparse analytical Jacobian entries for $\delta^\prime$ and $\theta^\prime$,
\item output and interpolate any variable $S(\tau,k)$ from the solution.
\end{itemize}

All these steps require some degree of manual intervention in a purely numerical code.
In CLASS one would have to
...
The changes are scattered across the code, so it deteriorates as more models are added.
%Moreover, a practical analysis would require both modifying the Boltzmann solver codebase and scripting new analysis code.
SymBoltz opens for a workflow to implement and analyze a modified model with less effort all in one notebook, for example.

Both codes already have a $w_0 w_a$ species, but this is how they are implemented.
...
\midrule
SymBoltz (no approximations) & 0.3 s & 4.3 s \\
CLASS (only tight-coupling approximation) & 7.6 s & 14.1 s \\
CLASS (all approximations) & 0.6 s & 2.4 s \\
\bottomrule
\end{tabularx}
\end{table}
...
Cls = spectrum_cmb([:TT, :TE, :EE], prob, jl)
\end{Verbatim}
\end{codebox}
...
It is similar CAMB vs. CLASS with standard precision,
...
Near a peak $\boldsymbol{\theta}_0$, where derivatives vanish, a log-likelihood $\log L$ of parameters $\boldsymbol{\theta}$ is approximated by the Taylor series
\begin{equation}
    \log L(\boldsymbol{\theta}) \approx \log L(\boldsymbol{\theta}_0) - \sum_{i,j} F_{ij}(\boldsymbol{\theta}_0) (\theta_i - \bar{\theta}_i) (\theta_j - \bar{\theta}_j),
\label{eq:likelihood_expansion}
\end{equation}
where $\boldsymbol{F}$ is the Fisher (information) matrix with elements
\begin{equation}
    F_{ij}(\boldsymbol{\theta}) = -\frac12 \frac{\partial^2 \log L(\boldsymbol{\theta})}{\partial \theta_i \, \partial \theta_j} .
...
Fisher forecasting is powered by the Cramér-Rao bound, which guarantees that $\big\lvert \smash{F^{-1}_{ij}} \big\rvert \leq \big\lvert \smash{C_{ij}} \big\rvert$ bounds the covariance $C_{ij}$ between parameters $\theta_i$ and $\theta_j$.
The inequality is saturated as the approximation \eqref{eq:likelihood_expansion} improves, and becomes an equality for a Gaussian.
Thus, inverting $\boldsymbol{F}(\boldsymbol{\theta}_0)$ gives the tightest parameter constraints one can hope for.
...
\end{equation}
%the measurements of each $\bar{C}_l$ are normally distributed and uncorrelated, the log-likelihood for this experiment is $\chi^2$:
Assuming a $\chi^2$-log-likelihood
\begin{equation}
    \log L(\boldsymbol{\theta}) = -\frac12 \sum_l \bigg( \frac{C_l(\boldsymbol{\theta}) - \bar{C}_l}{\sigma_l} \smash{\bigg)^2} ,
\end{equation}
the Fisher matrix \eqref{eq:fisher_general} becomes
\begin{equation}
    F_{ij} = \sum_l \frac{\partial C_l}{\partial \theta_i} \frac{1}{\sigma_l^2} \frac{\partial C_l}{\partial \theta_j} .
\label{eq:fisher_special}
\end{equation}

The derivatives $\partial C_l / \partial \theta_i$ are nontrivial to compute.
They are usually found with error-prone finite differences by carefully tuning the step.
SymBoltz skips this problem altogether by differentiating the function for $C_l(\boldsymbol{\theta})$ with ForwardDiff.jl:
\begin{codebox}
\begin{Verbatim}
vary = [
...
dCl_dθ = ForwardDiff.jacobian(Cl, θ₀)
\end{Verbatim}
\end{codebox}
Here \texttt{vary} orders the parameters to differentiate with respect to,
and \texttt{genprob} generates a new problem with updated parameters \texttt{θ}.
This sets up a vector-to-vector function \texttt{Cl(θ)} is differentiated by \texttt{ForwardDiff.jacobian}
to compute $\partial C_l / \partial \theta_i$ with dual numbers, as shown in \cref{fig:derivatives}.
...
Finally, we show that SymBoltz outputs differentiable results for gradient-based MCMC samplers, like the No-U-Turn Sampler (NUTS).
It uses gradients to sample parameter space more efficiently than the \enquote{blind} Metropolis-Hastings algorithm.

We predict apparent magnitudes $m(z) = M + \mu(z)$ of Type Ia supernovae at redshift $z$ and fit to 1048 Pantheon observations $(z_i, m_i)$ \citep{scolnicCompleteLightcurveSample2018}.
...
The differentiable computation is not yet fast enough for perturbation-derived spectra.
While these results are not groundbreaking, they are a first step to computing fast gradients through the entire solver.
More advanced MCMCs with a differentiable background have been done by \cite{campagneJAXCOSMOEndtoEndDifferentiable2023}, for example.
We hope to improve this with future work (see \cref{sec:futurework}).
...
\begin{figure}
    ...
    \caption{%
        ...
        SymBoltz is approximation-free and run with the implicit ODE solvers \texttt{TRBDF2}/\texttt{KenCarp4}/\texttt{Kvaerno5}/\texttt{FBDF}.
        CLASS is run with approximations on/off and its implicit/explicit evolvers \texttt{ndf15}/\texttt{rk}, but tight coupling is always approximated.
        For each setup, $P(k)$ is computed by integrating perturbation $k$-modes with ODE solver tolerances $10^{-2}$-$10^{-9}$ that controls the adaptive time stepping.
        ...
        Both codes solve the same $w_0 w_a \text{CDM}$ model with equal parameters, $N=114$ identical wavenumbers $k_i$, multipole cutoff $\lmax=16$ for all species, sample massive neutrino momenta with default settings and parallelize over $k$.
        The timings are from a Linux laptop with an Intel i7-12800H CPU.
        See \cref{sec:performance} and \cref{sec:precision} for more details.
    }
    \label{fig:workprec}
\end{figure}
...
%Boltzmann solvers have a great deal of optimizations and tricks that are controlled by several hyperparameters, so such a comparison only makes sense in an apples-to-apples configuration.
...
The comparison shows how approximations and the choice of ODE method affects integration of the Einstein-Boltzmann equations.
...
When CLASS uses its approximation schemes and explicit evolver, SymBoltz is about as fast for most precision requirements while remaining approximation-free.
%CLASS' performance could improve with more efficient $q$-sampling, and SymBoltz' speed could likewise improve with implementation of approximation schemes at the cost of code complexity.

To make this comparison as apples-to-apples as possible, we configured both codes to integrate identical $k$-modes (decided by CLASS) without $k$-interpolation and use the same multipole cutoff $\lmax$ for all species.
However, the codes sample massive neutrino momenta $q$ differently.
This has a major impact on the number of perturbation equations and the work needed to solve them (see \cref{sec:massiveneutrinos}).
SymBoltz defaults to 4 momenta and gives 127 perturbation equations in total.
CLASS' default $\texttt{tol\_ncdm\_newtonian} = 10^{-5}$ gives 11 momenta and 245 equations (before approximations).
Increasing this tolerance to $10^{-3}$ gives 5 momentum bins, 143 equations and is around $245/143 \approx 1.7$ times faster, but introduces errors beyond $1\%$ in $P(k)$ in \cref{fig:spectra}.
We therefore leave both codes with default $q$-sampling, but note that we think CLASS could be up to $2\times$ faster at unchanged precision with more efficient $q$-sampling.
This would shift the curves in \cref{fig:workprec} down by $\lg 2 \approx 0.3$.
Similarly, \cite[appendix A]{howlettCMBPowerSpectrum2012} find a precise 3-4 point scheme in CAMB, and \cite{leeEfficientAnalyticApproximation2025} speed up CLASS with an integral equation approach for massive neutrinos in the fork CLASSIER\footnote{\url{https://github.com/nanoomlee/CLASSIER}}.
% https://github.com/cmbant/CAMB/blob/06a0845/fortran/massive_neutrinos.f90#L84-L97

SymBoltz is currently slower than CAMB and CLASS for computing CMB power spectra $C_l$.
...
These optimizations are very well polished by CAMB and CLASS, and unmatched by SymBoltz at this time.
A similar detailed performance analysis for $C_l$ is therefore not meaningful now.
It would also blur the impact and viability of approximation-free perturbations, which is SymBoltz' first design milestone.
With more work, the $C_l$ optimizations can be implemented in SymBoltz.

\section{Future work}
\label{sec:futurework}

We hope to continue to improve SymBoltz with new features and enable others to build upon it.
In particular, its symbolic capabilities could be extended with utility methods for:
\begin{itemize}
\item gauge transformation of equations, say from the Newtonian gauge in SymBoltz to the synchronous gauge used by default in CAMB and CLASS, as already featured in CAMB\footnote{\url{https://camb.readthedocs.io/en/latest/symbolic.html}};
\item perturbation theory for deriving initial conditions and approximation schemes for new models;
%\item initial conditions for perturbations generated with computer-assisted symbolic perturbation theory;
%\item approximation schemes generated with computer-assisted symbolic perturbation theory, to enable speedups without costly reinvention of approximations with any model change;
...
We also aim to improve performance, particularly of differentiable parameter runs.
An attractive milestone is efficient reverse-mode automatic differentiation of scalar loss functions, which could enable very powerful gradient-based MCMC sampling.
Support for sparse (Jacobian) matrices is also on our radar, and could accelerate the solution of large perturbation systems.
...
In a larger context, we are hopeful that a Boltzmann solver in Julia can foster growth of modular interoperating add-on packages.
For example, applications like higher-order perturbation theory (e.g. CLASS-PT by \cite{chudaykinNonlinearPerturbationTheory2020} and PyBird by \cite{damicoLimits$w$CDMEFTofLSS2021}), nonlinear boosting (e.g. HALOFIT by \cite{takahashiRevisingHalofitModel2012} and HMCODE by \cite{meadHMcode2020ImprovedModelling2021}), CMB lensing (e.g. CMBLensing.jl by \cite{milleaBayesianDelensingDelight2020}) and nonlinear $N$-body simulations all rely on the output from Boltzmann solvers.
It is possible to make such existing codes compatible, but supporting automatic differentiation across language barriers is challenging.
An ecosystem of similar packages in one language with good support for automatic differentiation, like Julia or Python combined with Jax, could open the door to a larger, modular and fully differentiable cosmological modeling toolbox.
A recent example of this is the impressive end-to-end modeling from linear perturbations to non-linear structure formation with a differentiable Boltzmann code coupled to a differentiable $N$-body code by DISCO-DJ \citep{hahnDISCODJDifferentiableEinsteinBoltzmann2024,listDISCODJIIDifferentiable2025}.
...
It relaxes all approximation switching schemes found in older codes by solving a single set of stiff equations at all times with implicit integrators.
This simplifies the equations, and fast generated code for ODEs and analytical and sparse Jacobians preserves the speed of approximation-based codes.
It is differentiable, so one can get accurate derivatives of any output quantity with respect to any input parameter.
Output power spectra agrees with CLASS to $0.1\%$, which is the same level of accuracy found between CAMB and CLASS.
...
\begin{figure}
    ...
    \node[pillar, fill=blue!50] (app) at (240:5.8cm) {Approximation-\\freeness};
    \node[pillar, fill=red!50] (dif) at (300:5.8cm) {Differentiability};
    ...
    \caption{%
        ...
        For example: differentiability helps to calculate the ODE Jacobian accurately and efficiently; which is needed by implicit solvers to integrate stiff ODEs without approximations; which makes it easier to write down models in a simple symbolic form; which can be used to detect the sparsity pattern of the Jacobian; which in turn increases the efficiency of the implicit ODE solver.
        This creates a self-reinforcing design.
    }
    \label{fig:synergy}
\end{figure}

Some of these main features -- a symbolic-numeric interface, approximation-freeness and differentiability -- are notably also implemented in other recent Boltzmann codes like DISCO-EB, Bolt.jl and PyCosmo.
SymBoltz combines them into one code and uses a synergy between them that reinforces its design, as illustrated in \cref{fig:synergy}.
Together, these alternative codes now offer a simpler approximation-free structure in high-level languages
that complements traditional codes like CAMB and CLASS that are highly optimized around historical approximation schemes and low-level languages.
However, new codes need more work to reach the level of features available in CAMB and CLASS that have grown over many years.

SymBoltz version 0.14.0 includes the metric in the flat Newtonian gauge, General Relativity, cold dark matter, photons, baryons and RECFAST recombination, massless and massive neutrinos, the cosmological constant and $w_0 w_a$ dark energy with scalar perturbations, luminosity distances and matter and CMB spectra.
More models and observables, vector and tensor perturbations and curved geometry are left for future work.
Forward-mode automatic differentiation works for single-run sensitivity analyses like Fisher forecasting.
A next milestone is reverse-mode for scalar loss functions, particularly for perturbation-derived spectra.
This could challenge emulators in smarter gradient-based MCMC sampling.
No differentiable Boltzmann code has yet demonstrated this ability.

SymBoltz is easy to install from \url{https://github.com/hersle/SymBoltz.jl}.
Documentation\footnote{\url{https://hersle.github.io/SymBoltz.jl/}} is available there, and the code is tested with continuous integration to ensure it remains correct and up-to-date (see \cref{sec:testing}).
\textbf{Anyone is encouraged to ask questions, give feedback, open issues and contribute pull requests in the repository!}
We hope others can find it useful to build tools on top of SymBoltz, and that it offers valuable competition and new ideas on the Boltzmann solver market.

\section*{Acknowledgments}

I thank the entire Julia community for striving towards an open ecosystem of scientific packages upon which SymBoltz continues to grow.
...
I thank Hans A. Winther for helpful suggestions, testing the code and giving feedback on it and drafts of this paper.
I thank Julien Lesgourgues, Thomas Tram and others for developing CLASS and inspiring SymBoltz.
...
CLASS and SymBoltz are configured to use the same Newtonian gauge, $\lmax$ for all species, RECFAST recombination model, $\tanh$-like reionization parametrization, $w_0 w_a$ dark energy equations and cosmological parameter values.
...
Its default value was changed from 3000 to 40000 in 2023, but we suspect that the increased density in points makes the splines oscillate from numerical noise. % https://github.com/lesgourg/class_public/commit/4e1788ddfcb80db3097cddd02ddf332c51cc15e7
