...
function solve(M::CosmologyModel, pars; aini = 1e-7, aend = 1e0, solver = Rodas5P(), reltol = 1e-13, kwargs...)
    ...
    tend = tini + Δt
    aini = bg_sol[M.g.a][end]
    th_prob = ODEProblem(M.th, [M.g.a => aini], (tini, tend), pars; use_union = false)
...
function solve(M::CosmologyModel, pars, ks::AbstractArray; aini = 1e-7, aend = 1e0, solver = KenCarp47(), reltol = 1e-9, verbose = false, kwargs...)
    ...
    if :b₊rec₊dτspline in Symbol.(parameters(M.pt))
        ...
        pars = [pars;
            ...
            #M.b.rec.cs²spline => spline(log.(th_sol(ts, idxs=M.b.rec.Tb).u), log.(ts)),
        ]
    end

    ki = 1.0
    ode_prob0 = ODEProblem(M.pt, [M.g.a => aini], (tini, tend), [pars; k => ki]) # TODO: why do I need this???
    ...
    ode_probs = EnsembleProblem(; safetycopy = false, prob = ode_prob0, prob_func = (ode_prob, i, _) -> begin
        ...
        return ODEProblem(M.pt, [M.g.a => aini], (tini, tend), [pars; k => ks[i]]; use_union = false) # TODO: use remake https://github.com/SciML/OrdinaryDiffEq.jl/pull/2228, https://github.com/SciML/ModelingToolkit.jl/issues/2799 etc. is fixed
