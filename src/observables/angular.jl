...
"""
    spectrum_cmb(modes::AbstractVector{<:Symbol}, prob::CosmologyProblem, jl::SphericalBesselCache; normalization = :Cl, unit = nothing, kinterp = nothing, Δkτ0 = 2π/4, xs = cosgrid(0.0, 1.0; length=600), τcut = 1e-2, l_limber = 11, bgopts = (alg = bgalg(prob), reltol = 1e-7, abstol = 1e-7), thopts = (alg = thalg(prob), reltol = 1e-7, abstol = 1e-7), ptopts = (alg = ptalg(prob), reltol = 1e-5, abstol = 1e-5), thread = true, verbose = false, kwargs...)
...
- `bgopts`/`thopts`/`ptopts`: ODE precision parameters for `bg`/`th`/`pt` stages.
...
function spectrum_cmb(modes::AbstractVector{<:Symbol}, prob::CosmologyProblem, jl::SphericalBesselCache; normalization = :Cl, unit = nothing, kinterp = nothing, Δkτ0 = 2π/4, xs = cosgrid(0.0, 1.0; length=600), τcut = 1e-2, l_limber = 11, bgopts = (alg = bgalg(prob), reltol = 1e-7, abstol = 1e-7), thopts = (alg = thalg(prob), reltol = 1e-7, abstol = 1e-7), ptopts = (alg = ptalg(prob), reltol = 1e-5, abstol = 1e-5), thread = true, verbose = false, kwargs...)
    ...
    ls = collect(jl.l)
    sol = solve(prob; bgopts, thopts, verbose)
    ...
    τs = sol.th.t # by default, use background (thermodynamics) time points for line of sight integration
    τs = τs[τs .≥ τcut]
    ...
    Ss = source_grid(prob, Ss, τs, ks_fine, kinterp, sol.bg, sol.th; ptopts, verbose, thread)
    if iψ > 0
        ...
        τrec = sol.th.t[argmax(sol[prob.M.b.v])]
