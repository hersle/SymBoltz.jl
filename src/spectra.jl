...
function integrate(xs, ys; integrator = SimpsonsRule())
...
"""
    los_integrate(S0s::AbstractArray{T}, S1s::AbstractArray{T}, ls::AbstractArray, ks::AbstractRange, ts::AbstractArray, us::AbstractRange, u′s::AbstractArray; integrator = SimpsonsRule(), verbose = false) where {T <: Real}
...
"""
function los_integrate(S0s::AbstractArray{T}, S1s::AbstractArray{T}, ls::AbstractArray, ks::AbstractRange, ts::AbstractArray, us::AbstractRange, u′s::AbstractArray; integrator = SimpsonsRule(), verbose = false) where {T <: Real}
...
"""
    spectrum_cmb(ΘlAs::AbstractArray, ΘlBs::AbstractArray, P0s::AbstractArray, ls::AbstractArray, ks::AbstractRange; integrator = SimpsonsRule(), normalization = :Cl)

Compute ``Cₗᴬᴮ = (2/π) ∫dk k^2 P₀(k) Θₗᴬ(k) Θₗᴮ(k)`` for the given `ls`.
If `normaliation == :Dl`, compute ``Dₗ = Cₗ * l * (l+1) / 2π`` instead.
"""
function spectrum_cmb(ΘlAs::AbstractArray, ΘlBs::AbstractArray, P0s::AbstractArray, ls::AbstractArray, ks::AbstractRange; integrator = SimpsonsRule(), normalization = :Cl)
...
"""
    spectrum_cmb(modes::AbstractArray, prob::CosmologyProblem, ls::AbstractArray; integrator = SimpsonsRule(), normalization = :Cl, unit = nothing, kwargs...)
...
"""
function spectrum_cmb(modes::AbstractArray, prob::CosmologyProblem, ls::AbstractArray; integrator = SimpsonsRule(), normalization = :Cl, unit = nothing, kwargs...)
