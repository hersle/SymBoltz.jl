...
function thermodynamics_recombination_recfast(g; reionization = true, Hflag = 1, Heflag = 6, kwargs...)
    Hflag ∈ [0, 1] || error("Supported H flags are 0, 1. Got $Hflag.")
    Heflag ∈ [0, 1, 2, 3, 6] || error("Supported He flags are 0, 1, 2, 3, 6. Got $Heflag.")
    ...
    if Hflag == 0 # always true
        fudgeH = 1.14
    end
    if Hflag == 1
        fudgeH -= 0.015 # https://github.com/lesgourg/class_public/blob/5668031a8021bdc399e380ecf58f7a8e37a2a5e4/external/RecfastCLASS/wrap_recfast.c#L64
    end

    αH_fit(T; F=fudgeH, a=4.309, b=-0.6166, c=0.6703, d=0.5300, T₀=1e4) = F * 1e-19 * a * (T/T₀)^b / (1 + c * (T/T₀)^d) # fitting formula to Hummer's table (H fudge factor here is equivalent to the way RECFAST does it)
    ...
    defaults = [
        ...
        XH⁺ => 1.0 #e- αH/βH # + O((α/β)²); from solving β*(1-X) = α*X*Xe*n with Xe=X
        _κ => 0.0
        κ0 => NaN
        re1z => 7.6711
        re2z => 3.5
    ...
    eqs = [
        ...
        KH1 ~ KH0 * KH_KH0_fit(g.a) # TODO: if Hflag
        KH ~ KH0 + KH1
        ...
        KHe ~ 1 / (KHe0⁻¹ + KHe1⁻¹ + KHe2⁻¹) # corrections to inverse KHe are additive # TODO: KHe⁻¹?
        CHe ~ (exp(-βb*E_He_2p_2s) + KHe*ΛHe*nHe*(1-XHe⁺+1ϵ)) /
              (exp(-βb*E_He_2p_2s) + KHe*(ΛHe+βHe)*nHe*(1-XHe⁺+1ϵ))
    ...
    if reionization
        ...
        push!(eqs, re1Xe ~ 0, re2Xe ~ 0)
    end

    if Heflag ≥ 0 # always true
        push!(eqs, KHe0⁻¹ ~ (8π*g.H) / λ_He_2p_1s^3)
    end

    if Heflag ≥ 1
        # RECFAST He flag 1 (close to zero modification?) # TODO: not that good, reliability depends on ϵ to avoid division by 0; try to use proper Saha ICs with XHe⁺ ≠ 1.0 and remove it
        append!(eqs, [
    ...
    # RECFAST He flag 2 (Doppler correction) # TODO: increase reliability, particularly at initial time
    if Heflag ≥ 2
        append!(eqs, [
    ...
    if Heflag ≥ 3
        ...
        append!(eqs, [
            ...
            τHe3 ~ A2Pt*nHe*(1-XHe⁺+ϵ)*3 * λ_He_2p_1s_tri^3/(8π*g.H) # TODO: last g.H is an error?
            pHe3 ~ (1 - exp(-τHe3)) / τHe3 # softplus?
        ...
        push!(eqs, DXHe⁺_triplet ~ 0) # no triplet correction
    end
