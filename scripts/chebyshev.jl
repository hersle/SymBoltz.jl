...
yticks = 0:500:3000
margin = -20*Plots.mm
pT = surface(sol, M.τ, M.k, (M.k*M.χ)^2*M.ST; xlims = (0.035, 0.085), xticks = 0.04:0.01:0.08, ylims, yticks, camera, size = (550, 450), margin)
savefig(pT, "paper_chebyshev/figures/ST.pdf")
pE = surface(sol, M.τ, M.k, (M.k*M.χ)^2*M.SE; xlims = (0.035, 0.085), xticks = 0.04:0.01:0.08, ylims, yticks, camera, size = (550, 450), margin)
savefig(pE, "paper_chebyshev/figures/SE.pdf")
pψ = surface(sol, M.τ, M.k, -(M.k*M.χ)^2*M.Sψ; ylims, yticks, camera, size = (550, 450), margin)
savefig(pψ, "paper_chebyshev/figures/Spsi.pdf")
pm = surface(sol, M.τ, M.k, M.m.Δ; ylims, yticks, camera, size = (550, 450), margin)
...
Ss_cheb = source_grid_chebyshev(prob, S, τs, ks_raw, korder, sol.bg)
...
Ss_cubic = source_grid(prob, S, τs, ks_cubic, sol.bg)
...
for klength in klengths
    ...
    korder = klength - 1
    Ss = source_grid_chebyshev(prob, S, τs, ks_raw, korder, sol.bg)
...
τ = τs[argmax(sol[M.b.v])] # peak of visibility function
Ss_raw = source_grid(prob, S, [τ], ks_raw, sol.bg)
...
for (color, klength) in enumerate(10:10:70)
    # 1) Chebyshev interpolation at chebyshev k-nodes
    korder = klength - 1
    Ss = source_grid_chebyshev(prob, S, [τ], ks_raw, korder, sol.bg)
    Ss = Ss[1, :]
    ...
    plot!(pinterp_cheb, ks_raw, abs.(Ss .- Ss_raw); color, xlabel = M.k, ylabel = "error", yscale = :log10, ylims = (1e0, 1e9), label = nothing, subplot = 2)
end
pinterp_cheb
...
order = 200
τs = [τ]
interp = only(source_grid_chebyshev(prob, S, τs, extrema(ks_raw), (order,), sol.bg))
ns = 0:order
p = plot(ns, abs.(interp.coefs); yscale = :log10, xlabel = "n", ylabel = "|cₙ|", label = "τ = $τ", ylims = (1e0, 1e8), xticks = 0:10:order, yticks = 10.0 .^ (-8:1:8))
...
for (color, klength) in enumerate(10:10:70)
    ...
    Ss = source_grid(prob, S, [τ], ks, sol.bg)
    Ss = source_grid(Ss, ks, ks_raw) # interpolate with Cubic splines
    ...
    plot!(pinterp_cubic, ks_raw, abs.(Ss .- Ss_raw); xlabel = M.k, color, ylabel = "error", yscale = :log10, ylims = (1e0, 1e9), label = nothing, subplot = 2)
end
pinterp_cubic
...
for (color, klength) in enumerate([16,32,64,128])
    # 1) Chebyshev interpolation at chebyshev k-nodes
    korder = klength - 1
    Ss = source_grid_chebyshev(prob, log(S), [τ], ks_raw, korder, sol.bg; f = log, f⁻¹ = exp)
...
mode = :TT
kτ0min, kτ0max = 0.01, 2*jl.l[end] # 2*lmax for TT/EE, 10*lmax for ψψ
kτ0s_cheb = [kτ0min, kτ0max] # 2*lmax for TT/EE, 10*lmax for ψψ
normalization = :Dl
klength_ref = 2^10+1 # reference
klengths = [2^n+1 for n in 4:9] # 30:10:80 for TT/EE; 100:100:800 for ψψ
Dls_ref = spectrum_cmb(mode, prob, jl; normalization, kτ0s = kτ0s_cheb, kinterpolate = :chebyshev, korder = klength_ref - 1)
Dlss_cheb = [spectrum_cmb(mode, prob, jl; normalization, kτ0s = kτ0s_cheb, kinterpolate = :chebyshev, korder = klength - 1) for klength in klengths] # Chebyshev
kτ0s_cubic = [lingrid(kτ0min, kτ0max; length) for length in klengths]
Dlss_cubic = [spectrum_cmb(mode, prob, jl; normalization, kτ0s, kinterpolate = :cubic) for kτ0s in kτ0s_cubic] # cubic splines
Dlss = Dlss_cheb
p = begin
    labelfunc(klength) = "$klength " * (Dlss === Dlss_cheb ? "Chebyshev" : "cubic spline") * " k-points"
    xscale = :log10
    legend_position = :topleft
    plot(ls, Dls_ref; ylims = (0, 1e-9), title = "mode = $mode", xscale, color = :black, linewidth = 3, label = labelfunc(klength_ref), layout = grid(2, 1, heights=[0.75, 0.25]), size = (600, 500), legend_position, subplot = 1)
    ...
    plot!(ls, [abs.(Dls./Dls_ref.-1) for Dls in Dlss]; xscale, xlabel = "ℓ", ylabel = "error", color = permutedims(eachindex(klengths)), yscale = :log10, label = nothing, ylims = (1e-7, 1e-1), subplot = 2)
end
